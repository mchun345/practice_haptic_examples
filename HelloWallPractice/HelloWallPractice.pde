...
void onTickEvent(CountdownTimer t, long timeLeftUntilFinish){
  ...
  if (haply_board.data_available()) {
    ...
    //these seem to be vectors
    /* GET END-EFFECTOR POSITION (TASK SPACE) */
    angles.set(haply_2DoF.get_device_angles()); 
    pos_ee.set( haply_2DoF.get_device_position(angles.array())); //x?
    pos_ee.set(device2graphics(pos_ee)); //y?
    
    /* PHYSICS OF THE SIMULATION */
    f_wall.set(0, 0); //force of the wall to be exerted in case of effector penetration
    
    //penetration wall?
    pen_wall.set(0, (pos_wall.y - (pos_ee.y+r_ee))); //calculate distance between end effector and virtual wall, only y matters since wall is horizontial 
    
    //if distance btw end effector and wall is less than zero (pentration wall is the calc rep of this)
    if (pen_wall.y < 0) {
      f_wall = f_wall.add((pen_wall.mult(-k_wall))); //then set force of wall (counter force), kwall is the counter force, but is "fixed" at the moment
    }

    f_ee = (f_wall.copy()).mult(-1); //force of end effector multiple to positive again, does vector multiplication
    f_ee.set(graphics2device(f_ee)); //send force calculation back to end effector
  }

  /* update device torque in simulation and on physical device */
  haply_2DoF.set_device_torques(f_ee.array()); //seem to have to call haply_2DOF first before accessing its values below
...
void createpantograph() {
  ...
  float l_ani=pixelsPerMeter*l; //apparently length of graphic objects in metres, but is unused...
  float L_ani=pixelsPerMeter*L; //apparently length of graphic objects in metres, but is unused...
  float d_ani=pixelsPerMeter*d;  //maybe depth?
  float r_ee_ani = pixelsPerMeter*r_ee;  //angles probably
  ...
  //unfortunately no labelling to tell what part of pantograph it refers too ... have to guess
...
void update_animation(float th1, float th2, float x_E, float y_E){
  ...
  //handle part it seems, but preserves coordinate system by using matrix function it seems
  pushMatrix(); //apparently saves current coordinate system
  shape(handle,x_E+(d/12*pixelsPerMeter), y_E, 2*r_ee*pixelsPerMeter, 2*r_ee*pixelsPerMeter); //adjusts handle
  stroke(0); 
  popMatrix(); //apparently restores previous coordinate system
}
